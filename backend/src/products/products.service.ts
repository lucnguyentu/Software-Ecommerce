...
export class ProductsService {
  ...
  async addProductReview(
    ...
    try {
      ...
      // can allow user update their review
    ...
    try {
      ...
      const review = product.feedbackDetails.find(
        (reviews: { customerId: string }) =>
          reviews.customerId === user._id.toString(),
      );

      if (!review) {
        throw new Error("You haven't rated this product yet ");
      }

      return {
        message: 'Fetch review from a product successfully',
        success: true,
        result: review,
      };
    } catch (error) {
  ...
  async UpdateProductReview(
    ...
    try {
      ...
      const ratings: any[] = [];
      product.feedbackDetails.forEach(
        (comment: { customerId: string; rating: number }) =>
          comment.customerId === user._id.toString()
            ? ratings.push(rating)
            : ratings.push(comment.rating),
      );
