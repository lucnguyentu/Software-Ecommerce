...
import { Button, Card, Form, InputGroup, ListGroup, Row } from 'react-bootstrap';
...
const UpdateProduct: NextPage<ProductProps> = ({ product, productIdForUpdate }) => {
    const { addToast } = useToasts();
    const [productForm, setProductForm] = React.useState(initialForm);
    const [requirementName, setRequirementName] = React.useState('');
    const [requirementDescription, setRequirementDescription] = React.useState('');
    const [hightlight, setHightlight] = React.useState('');
    const [updateRequirementIndex, setUpdateRequirementIndex] = React.useState(-1);
    ...
    const handleRequirementAdd = () => {
        if (updateRequirementIndex > -1) {
            setProductForm({
                ...productForm,
                requirementSpecification: productForm.requirementSpecification.map((requirement, index) => {
    ...
    const handlerSubmitForm = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        try {
            setIsLoading(true);
            productIdForUpdate
            ...
            setIsLoading(false);
            router.push(`update-product?productId=${productIdForUpdate}`);
        }
        e.preventDefault();
    };
        ...
        <Card className="updateProductCard" style={{ padding: '15px', marginTop: '20px' }}>
            <Row>
                ...
                <Col>
                    <Form>
                        ...
                        <p style={{ color: '#10557a' }}>Product highlights are listed below:</p>
                        <ListGroup>
                            {productForm.highlights.length > 0 ? (
                                ...
                                <ListGroup.Item className="text-center">No Highlights</ListGroup.Item>
                            )}
                        </ListGroup>
                    </Form>
                </Col>
            ...
            <Row>
                ...
                <Col style={{ textAlign: 'end' }}>
                    ...
                    <Button variant="primary" type="submit" onClick={handlerSubmitForm} disabled={isLoading}>
                        {isLoading && (
...
export const getServerSideProps: GetServerSideProps<ProductProps> = async (context): Promise<any> => {
    try {
