...
import { Badge, Button, Card, Col, Dropdown, DropdownButton, Form, Nav } from 'react-bootstrap';
...
const Product: NextPage<ProductProps> = ({ product, relatedProducts }) => {
    ...
    const [allSkuDetails, setAllSkuDetails] = React.useState(product?.skuDetails || []);

    const [displaySku, setDisplaySku] = React.useState(
        product?.skuDetails
            ? product?.skuDetails.sort((a: { price: number }, b: { price: number }) => a.price - b.price)[0] || {}
            : {},
    );
            ...
            <Row className="firstRow">
                ...
                <Col sm={8}>
                    ...
                    <p className="productPrice">
                        ...
                        {displaySku?.price || '000'} {''} vnđ <span> </span>
                        <Badge bg="warning" text="dark">
                            {displaySku?.lifetime ? 'Lifetime' : getFormatedStringFromDays(displaySku.validity)}
                        </Badge>
                    </p>
                    <ul>
                        {product?.highlights &&
                            product?.highlights.length > 0 &&
                            product?.highlights.map((highlight: string, key: any) => <li key={key}>{highlight}</li>)}
                    </ul>
                    ...
                    <div>
                        {product?.skuDetails &&
                            ...
                            product?.skuDetails
                                .sort((a: { validity: number }, b: { validity: number }) => a.validity - b.validity)
                    ...
                    <Row>
                        <Col sm={3}>
                            <Nav variant="pills" className="flex-column">
                                ...
                                {product?.requirementSpecification && product?.requirementSpecification.length > 0 && (
                                    <Nav.Item>
                        ...
                        <Col sm={9}>
                            <Tab.Content>
                                ...
                                <Tab.Pane eventKey="third">
                                    <ReviewSection reviews={product.feedbackDetails || []} productId={product._id} />
                                </Tab.Pane>
    ...
    try {
        ...
        return {
            props: {
                ...
                relatedProducts: data?.result?.relatedProducts || ([] as unknown as Record<string, any[]>),
            },
        };
    } catch (error) {
        console.log(error);
