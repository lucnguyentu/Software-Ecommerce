...
const ProductItem: FC<IProductItemProps> = ({ userType, product }) => {
    ...
    const deleteProduct = async () => {
        try {
            ...
            if (error.response) {
                if (Array.isArray(error.response.data?.message) && Array.isArray(error.response?.data?.message)) {
    ...
    const uploadProductImage = async (e: any) => {
        try {
            ...
            const uploadProductImageRes = await Products.uploadProductImage(product._id, formData);
        ...
        <Col>
            <Card className="productCard">
                <Card.Img
                    ...
                    variant="top"
                    src={uploading ? 'https://www.ebi.ac.uk/training/progressbar.gif' : product?.image}
                />
                <Card.Body>
                    <Card.Title onClick={() => router.push(`/products/${product?._id}`)}>
                        {product.productName}
                    </Card.Title>
                    <StarRatingComponent name="rate2" editing={false} starCount={5} value={product?.avgRating || 0} />
                    <Card.Text>
                            ...
                            <span className="priceText">
                                {product?.skuDetails
                                    ? product?.skuDetails?.length > 1
                                        ...
                                        : `${product?.skuDetails?.[0]?.price || '000'} vnđ`
                                    : '000 vnđ'}{' '}
                            </span>
                        </span>
                    </Card.Text>
                    ...
                    {userType === 'admin' ? (
                        <div className="btnGrpForProduct">
                            <div className="file btn btn-md btn-outline-primary fileInputDiv">
                                <Upload />
                                <input type="file" name="file" className="fileInput" onChange={uploadProductImage} />
                            </div>
                            <Link href={`/products/update-product?productId=${product?._id}`}>
                                <button className="btn btn-outline-dark viewProdBtn">
                                    <Pen />
                                </button>
                            </Link>
                            <Button
