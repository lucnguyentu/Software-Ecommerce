...
const RegisterLogin: FC<IRegisterLoginProps> = ({ isResgisterForm = false }) => {
    ...
    const verifyUser = async (e: any) => {
        ...
        try {
            ...
            setIsLoading(true);
            const { success, message }: resposnePayload = await Users.verifyOTP(otpForm.otp, otpForm.email);
            if (!success) throw new Error(message);
            addToast(message, { appearance: 'success', autoDismiss: true });
            setOtpTime(false);
    ...
    const forgotPassword = async (e: any) => {
        ...
        try {
            ...
            if (!validator.isEmail(email)) {
                throw new Error('Invalid email. Plese enter a valid email and we will send you a password for you');
            }
            setIsLoadingForgotPwd(true);
            const { success, message }: resposnePayload = await Users.forgotUserPassword(email);
            if (!success) throw new Error(message);
            addToast(message, { appearance: 'success', autoDismiss: true });
        } catch (error: any) {
            if (error.response) {
    ...
    return (
        <Card>
            ...
            <Card.Body>
                ...
                {!isResgisterForm && (
                    <a style={{ textDecoration: 'none' }} href="" onClick={forgotPassword}>
                        {isLoadingForgotPwd && (
