...
import { Button, Card, Form, InputGroup, Badge, ListGroup } from 'react-bootstrap';
import { Archive, ArrowLeft, Check2Circle, Pen, Plus } from 'react-bootstrap-icons';
...
const SkuDetailsLicense: FC<ISkuDetailsLicenseProps> = ({ licensesListFor, setLicensesListFor, productId }) => {
    ...
    const deleteLicense = async (licenseId: string) => {
        try {
            const deleteConfirm = confirm('Want to delete? You will lost all licenses for this sku');
            if (deleteConfirm) {
    ...
    const addLicense = async () => {
        try {
            ...
            const addLicenseRes = licenseIdForUpdate
                ? await Products.updateLicense(productId, licensesListFor, licenseIdForUpdate, { licenseKey })
    ...
    return (
        <Card style={{ padding: '10px' }}>
            ...
            {addFormShow && (
                <Form>
                    <h6 style={{ color: 'green' }}>License Keys Information ({licenses.length})::</h6>
                    <Form.Group controlId="formBasicPassword">
                        ...
                        <InputGroup className="mb-3">
                            <Form.Control
                                ...
                                value={licenseKey}
                            />
                            <Button variant="secondary" onClick={addLicense} disabled={isLoading}>
                                {isLoading && (
                                    <span
                ...
                {licenses.length > 0 ? (
                    licenses.map((license: any, index: number) => (
                        <ListGroup.Item key={index}>
                            ...
                            <span className="delLBtn" onClick={() => deleteLicense(license._id)}>
