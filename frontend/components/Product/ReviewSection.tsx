...
const ReviewSection: FC<IProps> = ({ reviews, productId }) => {
    ...
    const [updateLoad, setUpdateLoad] = React.useState(false);
    ...
    const updateComment = (e: any, id: string) => {
        e.preventDefault();
        setFormShown(true);
        try {
        } catch (error) {
        ...
        // setReviewFrom({...reviewForm, rating: reviews.})
    };
        ...
        <div>
                            ...
                            <Form>
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    ...
                                    <StarRatingComponent
                                        ...
                                        onStarClick={(nextValue) => setReviewFrom({ ...reviewForm, rating: nextValue })}
                                    />
                                </Form.Group>
                                <Form.Group className="mb-3" controlId="formBasicPassword">
                                    ...
                                    <Form.Control
                                        ...
                                        onChange={(e) => setReviewFrom({ ...reviewForm, review: e.target.value })}
                                    />
                                </Form.Group>
            ...
            <div className="filterRating">
                ...
                <StarRatingComponent
                    ...
                    value={filterValue}
                    onStarClick={(number) => {
                        const filteredAllReview: Record<string, any>[] = filteredReviews.filter(
                            (value) => value.rating === number,
                        );
            ...
            <div className="reviewZone">
                ...
                {allReviews.map((review, index) => (
                    <Card bg="light" key={index} text="dark" style={{ width: '100%' }} className="mb-2">
                        <Card.Header className="reviewHeader">
                            <PersonFill className="personReview" />
                            {review.customerName}
                            <StarRatingComponent name="rate2" editing={false} starCount={5} value={review.rating} />
                        </Card.Header>
                        <Card.Body>
                            <Card.Text>
                                <span className="reviewDt">{review.updatedAt}</span> <br />
                                {review.feedbackMsg}
                            </Card.Text>
                            <div>
                                <Button
                                ...
                                <Button
                                    style={{ marginLeft: '5px', width: '4.5rem', height: '2.5rem' }}
                                    variant="danger"
