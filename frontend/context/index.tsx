...
type Context = {
    ...
    dispatch: (action: { type: string; payload: Record<string, any> | undefined }) => void;
    cartItems: any;
...
// cart reducer
const cartReducer = (state: any, action: { type: string; payload: Record<string, any> | undefined }) => {
    switch (action.type) {
        case 'ADD_TO_CART':
...
const Provider = ({ children }: Props) => {
    ...
    useEffect(() => {
        ...
        const cartItems = JSON.parse(window.localStorage.getItem('_digi_cart') || '[]');
    ...
    useEffect(() => {
        const getCsrfToken = async () => {
            const { data } = await axios.get(process.env.NEXT_PUBLIC_BASE_API_PREFIX + '/csrf-token');
    ...
    return <Context.Provider value={{ state, dispatch, cartItems, cartDispatch }}>{children}</Context.Provider>;
};
