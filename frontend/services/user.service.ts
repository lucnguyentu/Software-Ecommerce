...
export const Users = {
    ...
    loginUser: async (user: any): Promise<resposnePayload> => {
        ...
        window.localStorage.setItem('_digi_user', JSON.stringify(loginUserRes.result.user));
        return loginUserRes;
    },
    // update user details
    updateUser: async (user: any, id: string): Promise<resposnePayload> => {
        const updateUserRes = await requests.patch(`/users/update-name-password/${id}`, user);
        // get data from localStorage
        const userData = JSON.parse(window.localStorage.getItem('_digi_user') || '');
        // update user data
    ...
    forgotUserPassword: async (email: string): Promise<resposnePayload> => {
        const forgotUserPasswordRes = await requests.get(`/users/forgot-password/${email}`);
    ...
    verifyOTP: async (otp: string, email: string): Promise<resposnePayload> => {
        const verifyOTPRes = await requests.get(`/users/verify-email/${otp}/${email}`);
        return verifyOTPRes;
    },
